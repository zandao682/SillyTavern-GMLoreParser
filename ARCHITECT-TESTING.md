...
| # | Step | Verify |
|---|------|--------|
| P1 | gm-lore-parser v0.0.13 installed and enabled. | Console: `[gm-lore-parser] v0.0.13 loaded…`. |
| P2 | A local LLM connected with a **correct instruct template** and an adequate response-token / context budget (Stage 9 output is long). | A test generation completes without truncation. |
| P2.5 | **Disable any immersive/roleplay global System Prompt** (Advanced Formatting → System Prompt) while designing. A prose-roleplay system note (e.g. the "Roleplay - Immersive" preset: *"write at least one paragraph… be descriptive and immersive…"*) is prepended on top of the Architect's own prompt and **overrides structured output** — the model writes narration *about* emitting blocks instead of emitting them. Use a neutral/empty System Prompt for the Architect; keep the roleplay one for actual play. | A design turn that should emit blocks (e.g. Stage 9) emits real `[..._BEGIN]` tags, not prose like "I will now generate…". |
| P3 | Import `system-designer-card.json` (Characters → Import). Confirm `data.character_version` shows `0.0.13`. | Card present; version visible in card details. |
...
- *(Stage 8)* "Yep, that all matches what I want — go ahead and build it."
...
| ARC-15 | ARC-01 | World per brief | **Stage 1.5 System Definition**: asks features allow-list, attributes (+ optional descriptions), **progression model (levels/xp/none)**, derived formulas, optional classes, and the per-feature settings (reputation/capabilities incl. **require_granted**/needs/item_conditions/loyalty). Emits/echoes a `[SYSTEM_DEF]` for confirmation. |
| ARC-16 | ARC-15 confirmed | Vocab/possessions/locations/commands per brief | **Stage 1.6**: inventory model + **item_box**, equipment slots, optional location types/commands/vocabularies. |
| ARC-2 | ARC-16 | d100 roll-under per brief | **Stage 2**: captures the resolution mechanic, difficulty expression, crit/fumble → `resolution:` section. |
| ARC-3 | ARC-2 | accept | **Stage 3 / 3.2 / 3.5**: attributes recap, in-chat creation flow (CHAR_CREATE), and the **player stat-block schema** (field types, mutability, groups). No pre-pasted sheet. |
| ARC-45 | ARC-3 | skills points-tiers per brief | **Stage 4 / 4.5**: advancement triggers + **capability progression** (the `skill` category → a points-tiers profile in `progressions`). |
| ARC-5 | ARC-45 | a couple of NPCs; currency per brief | **Stage 5 / 5.5**: NPC/creature schema design; currency denominations + starting wealth. (Skips ranks/companions — not in features.) |
| ARC-6 | ARC-5 | accept; quests per brief | **Stage 6 / 6.25**: combat/conflict handling; quest categories/statuses. |
| ARC-65 | ARC-6 | header per brief | **Stage 6.5**: defines the `[HEADER_FORMAT]` (tokens only for fields this system has). |
| ARC-675 | ARC-65 | The Ashen Pact; reputation per brief | **Stage 6.75**: faction lore + reputation scale/tiers. |
| ARC-7 | ARC-675 | accept | **Stage 7**: GM guidance / pacing / WORLD_TIME triggers. (Skips 7.25/7.5 — world_events/domains off.) |
| ARC-8 | ARC-7 | accept | **Stage 8**: summarizes the **whole** system and asks for explicit confirmation. Produces **no** output yet. |
| ARC-9 | ARC-8 | "Yep, that all matches — go ahead and build it." | **Stage 9 (chunked)**: emits the card **one block per message** — `[CARD_BEGIN]` → several `[CARD_FIELD]` (the large `system_prompt` split across `append: true` chunks) → `[CARD_BOOK_ENTRY]`× (protocol + `[Campaign Setup]` + rules) → `[CARD_FINALIZE]`; then a one-shot `[LOREBOOK_OUTPUT]`. On `[CARD_FINALIZE]` the extension **assembles + downloads** the card (toast "Card … assembled & downloaded (N lore entries)"). Never dumps the whole card at once. |

## 6. Part A — guardrail negatives (`ARC-NEG-*`)

| ID | Check | Expected |
|----|----|----|
| ARC-NEG-01 | At any stage before ARC-9 confirmation, ask "just give me the card now." | Declines to emit output before Stage 8 confirmation (may offer a draft, clearly labelled). |
...
| PCARD-01 | Card: `spec == "chara_card_v2"`, `spec_version == "2.0"`, `data.name` set, **`data.character_version` present**. |
...
- **Stages → ARC IDs:** S0→ARC-00; S1→ARC-01; S1.5→ARC-15; S1.6→ARC-16; S2→ARC-2; S3/3.2/3.5→ARC-3; S4/4.5→ARC-45; S5/5.5→ARC-5; S6/6.25→ARC-6; S6.5→ARC-65; S6.75→ARC-675; S7→ARC-7; S8→ARC-8; S9→ARC-9.
...
1. **Stage 9 is chunked for a reason** — a single-shot full card is beyond small models (gemma-4-e4b emits a preamble then stalls). The Architect must emit `[CARD_BEGIN] → [CARD_FIELD]× → [CARD_BOOK_ENTRY]× → [CARD_FINALIZE]`, **one small block per message**, and split the large `system_prompt` with `append: true`. If the model tries to dump the whole card and truncates, nudge it to resume from the next `[CARD_FIELD]` (the buffer persists in `card_draft`), or restart Stage 9. Keep the design chat focused; long/poisoned history still hurts.
   - **Emit, don't narrate.** If the model replies "I will now emit…" *without a real block* (no `card_draft` opens), **check P2.5 first** — an immersive/roleplay global **System Prompt** is the most common cause: it orders descriptive prose and overrides the card's emit-blocks instruction at *any* temperature, so the model narrates instead of emitting. Disabling it flips the behavior immediately (verified: with the "Roleplay - Immersive" system prompt OFF, gemma-4-e4b's first Stage-9 turn began `[CARD_BEGIN] name: … [CARD_FIELD_BEGIN]…`; with it ON, it narrated regardless of temp 0.4/0.8/1.0 or instruct on/off). Secondary causes: history poisoning (accumulated narration turns → start a fresh chat) and over-long directives. Stage 9 is also written as direct imperatives; a literal "copy this opening" nudge reliably breaks a stall once the system prompt is clear.
   - **Early finalize is now blocked in code.** Small models may emit a few fields then jump to `[CARD_FINALIZE]`. The extension **refuses** the finalize (warning toast, draft left open) until `system_prompt` + `first_mes` + `post_history_instructions` + ≥1 book entry exist, so a broken card can no longer be produced — nudge the model to emit the missing pieces, then it finalizes. (The Stage-9 COMPLETENESS prompt rule still asks for the full set up front; the gate is the backstop.)
