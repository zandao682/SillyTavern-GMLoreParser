...
- ⚠️ Residual: the model wrote the attribute lines inline (`attributes|Brawn|BRN|…`) instead of the `attributes:` + indented `key | Label | ABBR` sub-format, so **attributes still wouldn't fully parse** without a tolerant parser or one more nudge; `[HEADER_FORMAT]` was omitted from the entry; and the produced `post_history` didn't include the block-discipline paragraph this pass (it did in the explicit run). These are the next polish targets — candidates for a small parser-tolerance tweak in the extension and/or a tighter Stage-8 template.
